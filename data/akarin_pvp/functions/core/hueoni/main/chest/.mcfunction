...
# アイテム補充
    ...
    execute as @e[type=armor_stand,tag=SetItem,sort=random,limit=5] at @s run function akarin_pvp:core/hueoni/main/chest/set_skill
    execute as @e[type=armor_stand,tag=SetItem,sort=random,limit=5] at @s run function akarin_pvp:core/hueoni/main/chest/set_totam
    execute as @e[type=armor_stand,tag=SetItem] run tag @s remove SetItem
